...
@interface FlutterIOSurfaceWrapper () {
  IOSurfaceRef _ioSurface;
}
@end

@implementation FlutterIOSurfaceWrapper

- (void)bindSurfaceToTexture:(GLuint)texture
                             fbo:(GLuint)fbo
                             size:(CGSize)size{
  [self createOrReplaceIOSurface:size];    
...
- (void)createOrReplaceIOSurface:(CGSize)size {
  if (_ioSurface) {
...
- (_Nonnull const IOSurfaceRef&)getIOSurface {
  return _ioSurface;
}
