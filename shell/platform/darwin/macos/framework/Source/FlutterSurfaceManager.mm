...
@interface FlutterSurfaceManager () {
  ...
  FlutterIOSurfaceWrapper* _ioSurfaces[kFlutterSurfaceManagerBufferCount];
...
- (instancetype)initWithLayer:(CALayer*)containingLayer
  ...
  if (self = [super init]) {
    ...
    _ioSurfaces[0] = [FlutterIOSurfaceWrapper alloc];
    _ioSurfaces[1] = [FlutterIOSurfaceWrapper alloc];
  }
  return self;
}
...
- (void)ensureSurfaceSize:(CGSize)size {
  ...
  for (int i = 0; i < kFlutterSurfaceManagerBufferCount; ++i) {
    [_ioSurfaces[i] createOrReplaceIOSurface:size];

    uint32_t fbo = [_frameBuffers[i] glFrameBufferId];
    uint32_t texture = [_frameBuffers[i] glTextureId];
...
- (void)swapBuffers {
  ...
  IOSurfaceRef contentIOSurface = [_ioSurfaces[kFlutterSurfaceManagerBackBuffer] getIOSurface];
